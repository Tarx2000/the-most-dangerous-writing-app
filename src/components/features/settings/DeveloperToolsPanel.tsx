...
import { FEATURE_FLAG_METADATA, DEFAULT_FEATURE_FLAGS, type FeatureFlags } from '@/config/flags';
import { getFeatureFlags, updateFeatureFlag, persistFeatureFlags } from '@/lib/featureFlags';
import type { SavedNote, Person, SavedVlog, AiQueueState, AiLogEntry, CompressionQueueState } from '@/types';
import { CompressionJobsPanel } from '@/components/features/settings/CompressionJobsPanel';
import {
    startConsoleCapture,
    stopConsoleCapture,
    getCapturedLogs,
...
const FeatureFlagsSection: React.FC = () => {
    ...
    return (
        ...
        >
            <Text style={{ color: theme.colors.gold, fontSize: 12, fontWeight: 'bold', marginBottom: 10 }}>
                🎛️ Feature Flags
            </Text>
            {(Object.keys(DEFAULT_FEATURE_FLAGS) as (keyof FeatureFlags)[]).map((key) => (
                <AnimatedScaleButton
                    key={key}
                    style={[
                        commonStyles.devToolBtn,
                        {
                            backgroundColor: flags[key] ? theme.colors.successBorder : theme.colors.glassBackground,
                            marginBottom: 8,
                        },
                    ]}
                ...
                >
                    <View style={commonStyles.devToolIconBox}>
                        <MaterialCommunityIcons
                            name={flags[key] ? 'check-circle' : 'circle-outline'}
                            size={16}
                            color={flags[key] ? theme.colors.green : theme.colors.textMuted}
                        />
                    </View>
                    <View style={{ flex: 1 }}>
                        <Text
                            style={[
                                commonStyles.devToolBtnText,
                                { color: flags[key] ? theme.colors.green : theme.colors.textMuted },
                            ]}
                        >
...
export const DeveloperToolsPanel: React.FC<DeveloperToolsPanelProps> = ({
    ...
    const relationshipTitlePromptRef = useRef(
        aiConfig.aiPrompts.relationshipTitle || DEFAULT_AI_PROMPTS.relationshipTitle,
    );
    const relationshipSummaryPromptRef = useRef(
        aiConfig.aiPrompts.relationshipSummary || DEFAULT_AI_PROMPTS.relationshipSummary,
    );
    ...
    return (
        <View
            style={{
                backgroundColor: preferences.devMode ? theme.colors.goldBorderLight : theme.colors.glassBackground,
                borderRadius: theme.borderRadius.md,
                padding: 20,
                marginBottom: 20,
                borderWidth: 1,
                borderColor: preferences.devMode ? theme.colors.goldBorder : theme.colors.glassBorder,
                marginTop: 10,
            }}
        >
            <Text
                style={[
                    ...
                    {
                        marginTop: 0,
                        color: preferences.devMode ? theme.colors.gold : theme.colors.textPrimary,
                        fontSize: 16,
                    },
                ]}
            >
            ...
            <AnimatedScaleButton
                style={[
                    commonStyles.closeVersionBtn,
                    {
                        backgroundColor: preferences.devMode
                            ? theme.colors.goldFillMedium
            ...
            >
                <Text style={[commonStyles.closeVersionBtnText, preferences.devMode && { color: theme.colors.gold }]}>
                    Dev Mode
                </Text>
                <Text
                    style={{
                        color: preferences.devMode ? theme.colors.gold : theme.colors.textMuted,
                        fontSize: 13,
                        fontWeight: 'bold',
                    }}
                >
                    {preferences.devMode ? 'ON' : 'OFF'}
                </Text>
            </AnimatedScaleButton>
            ...
            <AnimatedScaleButton
                style={[
                    commonStyles.closeVersionBtn,
                    {
                        backgroundColor: preferences.logMode
                            ? theme.colors.goldFillMedium
                            : theme.colors.glassHighlight,
                        marginTop: 8,
                        flexDirection: 'row',
            ...
            >
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <MaterialCommunityIcons
                        ...
                        size={16}
                        color={preferences.logMode ? theme.colors.gold : theme.colors.textMuted}
                    />
                    <Text
                        style={[commonStyles.closeVersionBtnText, preferences.logMode && { color: theme.colors.gold }]}
                    >
                        Verbose Logging
                    </Text>
                </View>
            ...
            {preferences.devMode && (
                <View style={{ marginTop: 15, gap: 10 }}>
                    ...
                    >
                        ...
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.gold }]}>
                    ...
                    >
                        ...
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.green }]}>
                            Run AI Benchmark
                        </Text>
                    </AnimatedScaleButton>
                    ...
                    >
                        ...
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.danger }]}>
                            Clear All Data
                        </Text>
                    </AnimatedScaleButton>
                    ...
                    >
                        ...
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.devBlue }]}>
                            Inspect AsyncStorage (Safe)
                        </Text>
                    </AnimatedScaleButton>
                        ...
                        onPress={async () => {
                            ...
                            try {
                                ...
                                if (result.skipped) {
                                    Alert.alert(
                                        'Safe Recovery',
                                        'No legacy data found in AsyncStorage. Nothing to recover.',
                                    );
                                } else {
                                    const errorMsg =
                    ...
                    >
                        ...
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.green }]}>
                            Safe Recover from AsyncStorage
                        </Text>
                    </AnimatedScaleButton>
                    ...
                    >
                        ...
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.devPurple }]}>
                            Restore from AsyncStorage (Destructive)
                        </Text>
                    </AnimatedScaleButton>
                        ...
                        onPress={async () => {
                            ...
                            try {
                                ...
                                if (result.orphans.length === 0) {
                                    Alert.alert(
                                        'Orphan Scan',
                                        'No orphaned video files found on disk.\n\nAll files are already tracked in the database.',
                                    );
                                } else {
                                    const list = result.orphans
                                        .map(
                                            (o) =>
                                                `• ${o.fileName} (${(o.fileSizeBytes / (1024 * 1024)).toFixed(1)} MB, ${o.modDate})`,
                                        )
                                        .join('\n');
                                    Alert.alert(
                    ...
                    >
                        ...
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.devOrange }]}>
                            Reset all AI Entries
                        </Text>
                    </AnimatedScaleButton>
                    ...
                    >
                        <Text style={{ color: theme.colors.gold, fontSize: 12, fontWeight: 'bold', marginBottom: 8 }}>
                            📊 Storage Info
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            Notes: {notes.savedNotes.length}
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            Circles: {personsHook.persons.length}
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            Current Streak: {streak.currentStreak}
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            Last Win: {streak.lastWinDate || 'Never'}
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            Streak History: {streak.streakHistory.length} days
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            Font: {CONFIG.FONTS[preferences.fontIndex]?.label || 'Default'} | Size:{' '}
                            {CONFIG.SIZES[preferences.sizeIndex]?.label || 'Default'}
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            Vlogs: {vlogs.savedVlogs.length} ({(vlogs.totalVlogStorageBytes / (1024 * 1024)).toFixed(1)}{' '}
                            MB)
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            AI Title Coverage: {notes.savedNotes.filter((n: SavedNote) => n.aiTitle).length}/
                            {notes.savedNotes.length} notes
                        </Text>
                        <Text style={{ color: theme.colors.textMuted, fontSize: 11 }}>
                            AI Queue: {queueState.pendingCount} pending, {queueState.isProcessing ? 'active' : 'idle'}
                        </Text>
                    </View>
                    ...
                    >
                        <View style={commonStyles.devToolIconBox}>
                            <MaterialCommunityIcons
                                name={showAiLog ? 'chevron-down' : 'format-list-bulleted'}
                                size={16}
                                color={theme.colors.gold}
                            />
                        </View>
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.gold }]}>
                            {showAiLog ? 'Hide' : 'Show'} App Log
                        </Text>
                    </AnimatedScaleButton>
                    ...
                    {showAiLog && (
                        ...
                        >
                            ...
                            >
                                <Text style={{ color: theme.colors.gold, fontSize: 12, fontWeight: 'bold' }}>
                                    App Log
                                </Text>
                                <View style={{ flexDirection: 'row', gap: 8 }}>
                                    ...
                                    >
                                        <Text style={{ color: theme.colors.danger, fontSize: 11, fontWeight: '600' }}>
                                            Clear AI
                                        </Text>
                                    </AnimatedScaleButton>
                                    ...
                                    <ScrollView style={{ maxHeight: 120 }} nestedScrollEnabled>
                                        {aiLogEntries.map((entry, i) => (
                                            <View
                                                ...
                                                style={{
                                                    borderBottomWidth: 1,
                                                    borderBottomColor: theme.colors.glassSurfaceSubtle,
                                                    paddingVertical: 3,
                                                }}
                                            >
                                                <Text style={{ color: theme.colors.textMuted, fontSize: 10 }}>
                                                    {new Date(entry.timestamp).toLocaleTimeString()} |{' '}
                                                    {entry.action.toUpperCase()} | {entry.phase}
                                                    {entry.durationMs ? ` | ${entry.durationMs}ms` : ''}
                                                </Text>
                                                {entry.error && (
                                                    <Text style={{ color: theme.colors.danger, fontSize: 10 }}>
                                                        {entry.error}
                                                    </Text>
                                                )}
                                            </View>
                    ...
                    >
                        <View style={commonStyles.devToolIconBox}>
                            <MaterialCommunityIcons
                                name={showFileSystem ? 'chevron-down' : 'folder-outline'}
                                size={16}
                                color={theme.colors.devBlue}
                            />
                        </View>
                        <Text style={[commonStyles.devToolBtnText, { color: theme.colors.devBlue }]}>
                            {showFileSystem ? 'Hide' : 'Show'} File System Explorer
                        </Text>
                    </AnimatedScaleButton>
                        ...
                        >
                            {viewingFile ? (
                                ...
                                <>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {currentPath !== FileSystem.documentDirectory && (
                                                ...
                                                >
                                                    <Text style={{ color: theme.colors.textPrimary, fontSize: 10 }}>
                                                        ⬅️ Up
                                                    </Text>
                                                </AnimatedScaleButton>
                                            )}
                                    ...
                                    <Text
                                        style={{ color: theme.colors.textMuted, fontSize: 8, marginBottom: 8 }}
                                        numberOfLines={1}
                                    >
                                        {currentPath}
                                    </Text>
                                            ...
                                            fileSystemData.map((file, i) => (
                                                    ...
                                                    onPress={async () => {
                                                        if (file.isDirectory) {
                                                            ...
                                                            try {
                                                                ...
                                                                setViewingFile({
                                                                    name: file.name,
                                                                    content: `[Cannot read file as text]\n${err}`,
                                                                });
                                                            }
                                                        }
                                                    }}
                                                >
                                                    ...
                                                    <Text style={{ color: theme.colors.textMuted, fontSize: 10 }}>
                                                        ...
                                                        {new Date((file.modificationTime || 0) * 1000).toLocaleString()}
                                                    </Text>
                                                </AnimatedScaleButton>
                                            ))
                                        )}
                    ...
                    >
                        ...
                        <TextInput
                            style={{
                                ...
                                borderRadius: 8,
                                borderWidth: 1,
                                borderColor: theme.colors.goldFill,
                                marginBottom: 8,
                                minHeight: 60,
                            ...
                            onEndEditing={() =>
                                aiConfig.saveAiPrompts({ ...aiConfig.aiPrompts, title: titlePromptRef.current })
                            }
                            multiline
                        />
                        <Text
                        ...
                        <TextInput
                            ...
                            onEndEditing={() =>
                                aiConfig.saveAiPrompts({ ...aiConfig.aiPrompts, summary: summaryPromptRef.current })
                            }
                            multiline
                        />
                        <Text
                        ...
                        <TextInput
                            ...
                            onEndEditing={() =>
                                aiConfig.saveAiPrompts({
                                    ...aiConfig.aiPrompts,
                                    relationshipTitle: relationshipTitlePromptRef.current,
                                })
                            }
                            multiline
                        />
                        ...
                        <TextInput
                            ...
                            onEndEditing={() =>
                                aiConfig.saveAiPrompts({
                                    ...aiConfig.aiPrompts,
                                    relationshipSummary: relationshipSummaryPromptRef.current,
                                })
                            }
                            multiline
                        />
                        ...
                        <TextInput
                            ...
                            onEndEditing={() =>
                                aiConfig.saveAiPrompts({ ...aiConfig.aiPrompts, grammar: grammarPromptRef.current })
                            }
                            multiline
                        />
                        <AnimatedScaleButton
                    ...
                    <CompressionJobsPanel
                        compressionState={compressionState}
                        onCancel={onCancelCompression}
                        onRetry={onRetryCompression}
                        onClearPending={onClearPendingCompressions}
                    />
                    ...
                    >
                        ...
                        <AnimatedScaleButton
                            style={[
                                commonStyles.devToolBtn,
                                {
                                    backgroundColor: preferences.debugLayout
                        ...
                        >
                            <View style={commonStyles.devToolIconBox}>
                                <MaterialCommunityIcons
                                    name={preferences.debugLayout ? 'eye' : 'eye-off'}
                                    size={16}
                                    color={preferences.debugLayout ? theme.colors.green : theme.colors.textMuted}
                                />
                            </View>
                            <Text
                                style={[
                        ...
                        <AnimatedScaleButton
                            ...
                            onPress={() => {
                                ...
                                setTimeout(() => {
                                    throw new Error('Developer Simulated Native Crash');
                                }, 100);
                            }}
                        >
                            <View style={commonStyles.devToolIconBox}>
                        ...
                        <AnimatedScaleButton
                            style={[
                                commonStyles.devToolBtn,
                                { backgroundColor: theme.colors.purpleFill, marginTop: 10 },
                            ]}
                            onPress={() => {
                                setShowSettings(false);
