...
import { DEFAULT_FEATURE_FLAGS, FEATURE_FLAG_STORAGE_KEY, type FeatureFlags } from '@/config/flags';
...
export function updateFeatureFlag<K extends keyof FeatureFlags>(key: K, value: FeatureFlags[K]): void {
    if (!_cachedFlags) {
        _cachedFlags = { ...DEFAULT_FEATURE_FLAGS };
    }
