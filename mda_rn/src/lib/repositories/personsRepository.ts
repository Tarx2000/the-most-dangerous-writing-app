...
export async function updatePerson(id: string, updates: Partial<Person>): Promise<void> {
    ...
    if (updates.customRelationships !== undefined) {
        fields.push('custom_relationships = ?');
        values.push(updates.customRelationships?.join(',') ?? null);
    }
