...
// Capture the REAL pure helpers (AiError, classifyError, classifyHttpStatus,
// isRetryableKind, isServerPersistentlyOffline) so the queue under test can
// classify errors faithfully. Only the network-touching functions
// (processNote, pingServer) are mocked.
//
...
// live bindings aren't enumerable own properties, which would drop the pure
// helpers and break classification.
...
describe('AI End-to-End (Mocked)', () => {
    ...
    const mockGetNoteById = (id: string) => (id === dummyNote.id ? dummyNote : undefined);
    ...
    beforeEach(async () => {
        ...
        (pingServer as jest.Mock).mockImplementation(() => Promise.resolve({ online: true }));
    });
    ...
    describe('full lifecycle', () => {
        it('should process a note from enqueue to updated note', async () => {
            ...
            await aiQueue.initialize(mockGetAiConfig, mockGetNoteById, mockUpdateNote, mockGetAllNotes);
            ...
            expect(logAi).toHaveBeenCalledWith(expect.objectContaining({ action: 'start' } as Record<string, unknown>));
            expect(logAi).toHaveBeenCalledWith(
    ...
    describe('retry path', () => {
        it('should retry transient failures up to AI_MAX_RETRIES then succeed', async () => {
            ...
            // Logger assertions
            expect(logAi).toHaveBeenCalledWith(expect.objectContaining({ action: 'fail' } as Record<string, unknown>));
            expect(logAi).toHaveBeenCalledWith(expect.objectContaining({ action: 'retry' } as Record<string, unknown>));
            expect(logAi).toHaveBeenCalledWith(
                expect.objectContaining({ action: 'success' } as Record<string, unknown>),
            );
    ...
    describe('cancellation mid-flight', () => {
        it('should cancel a job and not update the note', async () => {
            ...
            await waitForJobStatus((state) => state.currentJob?.status === 'processing', 2000);
            ...
            const aiUpdateCalls = (mockUpdateNote as jest.Mock).mock.calls.filter(
                (call: unknown[]) => typeof (call[1] as Record<string, unknown>)?.aiTitle === 'string',
            );
    ...
    describe('batch enqueue respects category ordering and newest-first', () => {
        it('should order jobs journal → circle → checkin with newest first', async () => {
            ...
            (pingServer as jest.Mock).mockImplementation(() => Promise.resolve({ online: false }));
