...
import { insertNote as repoInsertNote } from '@/lib/repositories/notesRepository';
import {
...
describe('createNotesOps', () => {
    ...
    beforeEach(() => {
        ...
        ops = createNotesOps(
            notesRef,
            setNotes as unknown as Setter<SavedNote[]>,
            streakRef,
            setCurrentStreak as unknown as Setter<number>,
            lastWinDateRef,
            setLastWinDate as unknown as Setter<string>,
            historyRef,
            setStreakHistory as unknown as Setter<string[]>,
        );
    });
    ...
    describe('saveNote', () => {
        ...
        it('should increment streak for won sessions >= 3 minutes', async () => {
            const note = {
                id: 'n1',
                text: 'This is a very long journal entry with many words so that it exceeds the tweet threshold and does not get auto classified as a tweet during the save operation which would then exclude it from streak counting logic entirely which we do not want here',
                dateStr: '2026-01-01',
                timestamp: Date.now(),
                durationMin: 5,
                won: true,
...
describe('createPersonsOps', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        personsRef = { current: [] };
        setPersons = jest.fn((val: Person[] | ((prev: Person[]) => Person[])) => {
            personsRef.current = typeof val === 'function' ? val(personsRef.current) : val;
        });
        notesRef = { current: [] };
        setNotes = jest.fn((val: SavedNote[] | ((prev: SavedNote[]) => SavedNote[])) => {
            notesRef.current = typeof val === 'function' ? val(notesRef.current) : val;
        });

        ops = createPersonsOps(
            personsRef,
            setPersons as unknown as Setter<Person[]>,
            notesRef,
            setNotes as unknown as Setter<SavedNote[]>,
        );
    });
...
describe('createFeedOps', () => {
    ...
    beforeEach(() => {
        ...
        ops = createFeedOps(
            bookmarksRef,
            setBookmarks as unknown as Setter<string[]>,
            commentsRef,
            setComments as unknown as Setter<Record<string, string>>,
            autoPlayRef,
            setAutoPlay as unknown as Setter<boolean>,
        );
    });
