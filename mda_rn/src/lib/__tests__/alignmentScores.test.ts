...
describe('alignmentScores', () => {
    ...
    describe('getAlignmentScoreColor', () => {
        ...
        it('matches color from full details', () => {
            for (const score of [1, 3, 5, 7, 9, 10]) {
                expect(getAlignmentScoreColor(score).color).toBe(getAlignmentScoreDetails(score).color);
            }
        });
    });
    ...
    describe('getAlignmentScoreFeed', () => {
        ...
        it('matches color from full details', () => {
            for (const score of [1, 3, 5, 7, 9, 10]) {
                expect(getAlignmentScoreFeed(score).color).toBe(getAlignmentScoreDetails(score).color);
            }
        });
    });
});
