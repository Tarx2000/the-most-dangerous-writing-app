...
- Large restores (1 GB+) extract checkpointed — one streaming pass per media file straight from its ZIP offsets (STORE = raw copy, DEFLATE = incremental chunked inflate, constant memory), per-file progress + stage callbacks. The central-directory scan never loads media bytes; NO nested isolates anywhere on the import path (each nesting level duplicates peak memory — measured +166 MB on the real archive). Media merges per-file into the existing dirs with a rollback journal (never rename-swap). Row `file_size_bytes` is refreshed to staged bytes; row↔manifest joins use the file basename (RN renames compressed videos: id ≠ filename). Failures always return a user-facing message, never a throw; media-less scoped imports keep existing videos.
