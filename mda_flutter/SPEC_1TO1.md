...
- Import gates: `.zip` only → extract → normalize (v2 + legacy v1, else reject) → **schema gate** (`backup.schemaVersion > current` → reject "update the app first") → **manifest gate** (all included present with exact size, else "corrupt backup") → **free-space gate** (`requiredBytes = manifestTotal × 2.5` vs native StatFs `availableBytes` via the `com.anonymous.mda_flutter/storage` MethodChannel, zero I/O; unknown → proceed, never block) → pause queues → safety snapshots (prefs pairs + DB file copy + vlog dir + thumbnail dir) → restore SQLite in ONE transaction (DELETE all, column-filtered re-insert) → rewrite media paths to sandbox → staged media extraction → **delete the own SAF temp-ZIP copy AFTER verified staging, BEFORE the merge** (`mda_backup_import_*.zip` in the temp dir only — user files are never deleted; never before extraction, the workers read from that file) → restore prefs allowlist (secret keys skipped, schema marker forced local) → merge media files (only media dirs present in the backup are touched; a media-less scoped import keeps existing videos) → success.
- Picker (Android): the system file picker runs through the native `com.anonymous.mda_flutter/backup_picker` channel (`MainActivity.kt`, `ACTION_OPEN_DOCUMENT`, `application/zip`). The document streams to `<cache>/mda_backup_import_<ts>.zip` in 4 MB chunks on a background thread with EventChannel progress (0..1, `-1` = done); only the cache path crosses back to Dart. `file_selector` is NEVER used for backup import on Android — its `toFileResponse()` allocates the whole file as one `byte[]` (1.3 GB → deterministic OOM in `onActivityResult`, proven by the S24 Ultra logcat) before any Dart code runs. Non-Android platforms keep the `file_selector` fallback.
- Media bytes are verified after staged extraction (truncated archives can pass header sizes); the whole ZIP decode runs off the UI isolate: media extracts in ONE worker isolate (top-level entry points `decodeBackupInIsolate` / `extractMediaInIsolate` calling static tear-offs with only primitive args — an inline instance-method closure would capture the service and transitively the live `WidgetsFlutterBinding`, killing the spawn on-device with "object is unsendable"; never nested per file), per-file progress (`onFileProgress(done, total)` → UI shows "Video X of Y") + stage callbacks. The central-directory scan never loads media bytes. Already-staged byte-exact files are skipped (resume after a kill). Media merges per-file into the existing dirs with a rollback journal (never rename-swap). Row `file_size_bytes` is refreshed to staged bytes; row↔manifest joins use the file basename (RN renames compressed videos: id ≠ filename). The SAF cache copy streams chunk-by-chunk with live progress (never whole-file in RAM); the screen stays awake via WakelockPlus during restore (released in `finally`); `android:largeHeap="true"` + `WAKE_LOCK` cover 1 GB+ restores on 12 GB phones. Failures always return a user-facing message, never a throw; media-less scoped imports keep existing videos.
...
- One shared session mode drives both Start hero and Library tab (RN `sessionMode`): nav taps switch content in place on whichever page is visible, never jump pages — the pager moves on horizontal swipe only. The Library header has NO tab pills; the section follows the mode (journal→notes, checkin→checkins, circles→circles, vlog→vlogs).
- Library header (RN `headerRow`): title 32/w600 + subtitle 16/secondary + live AI badge (processing only, spinner + pending count) on the left; morphing Unlock/Lock pill on the right — red pill with `Un`+`l` collapsing (17.5/4.5 px) and `L` expanding (7.2 px) over 250 ms cubic-out, shackle swinging open via rotateY 180° over 300 ms quad-out (never a flat spin). Sort dropdown (notes/check-ins only): bordered button, `Sort by: <BoldActive>` + chevron-down 20. Lock/unlock cross-fade via one spring (damping 30/stiffness 200): content 0.95→1 + 0→1 while the overlay slides up off-screen + 1→0. Lock cards: 48 px red lock, 22/w900 title, 15/22 subtitle, red pill 16/28 radius 100 + shadow (per-section copy: Notes/Circles/Vlogs Protected).
...
- **Calendar**: 7-col Monday-first; cell `(w−48)/7`, thumb height ×1.15; empty = 32 px circle; today = 2 px primaryAction ring; vlog days = dangerFill + dangerBorderMedium (today red border), day number 11/800, duration badge "m:ss", stack counter red 16; month swipe with spring commit ±6%. Day grouping is by LOCAL calendar day from `timestamp` (RN key `year-month-day`, each day newest-first) — never by the `dateStr` string, which carries German display text (`18.4.2026 14:20`) in RN backups. Missing thumbnails backfill lazily (RN `ThumbnailFetcher` parity). Circles/profile sheets are Navigator routes, never bare OverlayEntries; provider reads never happen in `initState` (controllers sync in `didChangeDependencies`).
