...
## Repairs implemented (2026-09-22 session — S24 Ultra import kill chain)

| Area | Repair | Evidence / remaining verification |
| --- | --- | --- |
| Backup isolate spawn failure (S24 Ultra logcat 22:37:43) | `❌ [STORAGE] Backup import failed Invalid argument(s): Illegal argument in isolate message: object is unsendable — _AsyncCompleter <- WidgetsFlutterBinding ...` — thrown BEFORE any backup byte was read. Root cause: `Isolate.run(() => _decodeBackup(zipPath))` created INSIDE an instance method captures `this` (the service + provider function fields), which transitively references the live `WidgetsFlutterBinding` via plugin-channel completers — unsendable, so the spawn dies on-device (tests with injected doubles never caught it: their closures are trivially sendable). Fix: four top-level entry points (`decodeBackupInIsolate`, `extractMediaInIsolate`, `writeExportZipInIsolate`, `verifyExportZipInIsolate`) calling static-method tear-offs (`BackupService._decodeBackup`) with only primitive args (String, List<Map>) — nothing unsendable can cross. New `_userFacingImportError` branch names this failure honestly ("background worker failed to launch") instead of blaming the backup file. | New regression test (`isolate entry points capture no UI state`, hostile non-sendable providers + real archive + full import). Full suite 188 green, analyzer clean. Release 1.5.14+2007 built; emulator re-verify (code as built): exact 1.2 GB user ZIP via `mdaTest.importBackup` → `success:true verification:ok`, 13+13 media, 43 notes / 13 vlogs / 17 persons, all 26 files logged `extracted … bytes`. Real S24 Ultra restore pending (user device). |
| Backup temp-ZIP deleted before extraction (latent, same session) | `_deleteOwnTempZip(zipPath)` ran BEFORE `_restoreMediaFiles(zipPath, …)` read from that same file (guard matches on-device: native picker writes `<cache>/mda_backup_import_*.zip`, `_tempDirProvider` IS the cache). Would have made staging read a ghost path → rollback → "could not be restored" on the very next attempt after the isolate fix. Fix: deletion moved to AFTER verified staging (byte-exact size gate), BEFORE the merge loop — the 3× disk peak (ZIP + staging + final) still never coexists; media-less imports skip deletion (nothing staged, no lane to free). | New test (`own temp ZIP survives extraction and is deleted only after staging`: media-carrying own-temp ZIP imports with `videosIncluded: 1`, file gone afterwards, staged video on disk; foreign files survive). Old no-media test reshaped accordingly. |

| Area | Repair | Evidence / remaining verification |
| --- | --- | --- |
| Backup free-space probe | The fill-probe WROTE up to ~1.4 GB of temp data just to "measure" capacity (slow, battery-hungry, could trigger the ENOSPC it checked for). Replaced by a native StatFs query via `com.anonymous.mda_flutter/storage` MethodChannel (`MainActivity.kt`, zero I/O, `-1` = unknown → proceed). Gate factor raised 1.1 → **2.5** (covers the transient ZIP + staging + final peak, not just final videos). | New behavior covered by existing gate tests (`freeSpaceProvider` seam unchanged); analyzer clean. Real low-storage device proof pending. |
| Backup media extraction | The per-file loop ran on the UI isolate with sync `readSync` + inflate (minutes of frozen screen → ANR-looking "crash", VM-service disconnects). The whole file loop now runs in ONE worker isolate (top-level `extractMediaInIsolate` entry point calling the static `_extractMediaWorkerWithProgress` tear-off — never nested per file, never an inline `this` closure); the UI only receives progress numbers. Already-staged byte-exact files are skipped (resume after a kill). Own SAF temp-ZIP copy deleted AFTER verified staging, BEFORE the merge (guarded: only `mda_backup_import_*.zip` inside the temp dir — user files never touched). | Backup suite 24 green (new: `onFileProgress` tick test, resume-skip test, own-temp-deletion test). Emulator re-verify (code as built): exact 1.2 GB user ZIP → `success:true verification:ok`, 13+13 media, 43/13/17 counts, **13/13 MD5 byte-exact**, app ALIVE, no FATAL/OOM/ANR in logcat. |
| Backup import UX | SAF cache copy (1.3 GB) sat on a frozen "Reading backup archive…" label — users killed the app thinking it crashed. Now: chunked copy with live progress (rescaled 0→0.1, UI yields every 4 MB), per-file "Video X of Y" counter + % during staging, screen kept awake via WakelockPlus (released in `finally`), `WAKE_LOCK` permission + `android:largeHeap="true"` in the manifest. | Widget-level: status/counter wiring covered by existing settings tests + analyzer; full tap-through on the S24 Ultra pending (user device). |
| Backup picker OOM (root cause, S24 Ultra logcat) | `FATAL ... OutOfMemoryError: Failed to allocate a 1295951840 byte allocation ... growth limit 536870912` inside `onActivityResult` — BEFORE any Dart backup code ran. Culprit: `file_selector_android`'s `toFileResponse()` (`new byte[size]` + `readFully()` + Pigeon `bytes` field = ~2× file size for a 512 MB heap). Fix per best practice: own SAF picker via `com.anonymous.mda_flutter/backup_picker` MethodChannel (`ACTION_OPEN_DOCUMENT`, `application/zip`) + `EventChannel` progress; native side streams the document to `<cache>/mda_backup_import_<ts>.zip` in 4 MB chunks on a background thread (`fd.sync()` + size check + `.zip` gate) and returns only the path. Dart `BackupPicker` wraps it (cancel → null, native errors → `BackupPickerException` with user-facing mapping in `settings_modal`); non-Android keeps the `file_selector` fallback. `file_selector` is no longer used for backup import on Android (kept as dep for export-share + fallback). | New `test/data/backup_picker_test.dart` (exception contract, stable error codes, cache-copy lifecycle/user-file-kept). Backup suite + picker green; analyzer clean. Release 1.5.14+2007 built + boot-verified on emulator. Real 1.3 GB S24 Ultra restore pending (user device). |
